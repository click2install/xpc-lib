...
import static io.xpchain.btc.Utils.decodeBase58;
...
public final class Address {
    ...
    Address(String address) throws BitcoinException {
        ...
        if (keyhashType == TYPE_MAINNET || keyhashType == TYPE_TESTNET || keyhashType == TYPE_P2SH || keyhashType == TYPE_P2SH_TESTNET) {
            if (Utils.verifyDoubleSha256Checksum(decodedAddress)) {
                witnessProgram = null;
                hash160 = new byte[20];
    ...
    static String publicKeyToAddress(boolean testNet, byte[] publicKey) {
        return ripemd160HashToAddress(testNet, Utils.sha256ripemd160(publicKey));
    }
    ...
    static String publicKeyToP2wkhAddress(boolean testNet, byte[] publicKey) {
        ...
        try {
            return Bech32.encodeSegwitAddress(testNet ? "txpc" : "xpc", 0, Utils.sha256ripemd160(publicKey));
    ...
    static String publicKeyToP2shP2wkhAddress(boolean testNet, byte[] publicKey) {
        ...
        return ripemd160HashToP2shAddress(testNet, Utils.sha256ripemd160(new Transaction.Script.WitnessProgram(0, Utils.sha256ripemd160(publicKey)).getBytes()));
    }
    ...
    private static String ripemd160HashToAddress(byte version, byte[] hashedPublicKey) {
        try {
            ...
            return Utils.encodeBase58(addressBytes);
